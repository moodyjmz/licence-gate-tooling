...
def main(argv, author=None):
    ...
    print(render(
        a=a, b=b, c=c, g=g,
        d_src=d_src, d_assets=d_assets, d_links=d_links, d_foreign=d_foreign,
        claims=claims, binary_skips=binary_skips, deleted_headers=deleted_headers,
        # Only where a register was actually in this diff. Telling a pull request
        # that never touched one that its rows survived is a claim about a file
        # this run never read - the shape of false all-clear the box exists to avoid.
        register_touched=any(
            NOTICE_FILE_RE.search(p)
            for p in list(added) + list(modified) + list(deleted)
            + [x for pair in renamed for x in pair]),
        head_short=resolve(head)[:9], author=author, notice=NOTICE))
